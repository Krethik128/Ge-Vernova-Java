...
public class TerminalOps {
    public static void main(String[] args) {
        ...
        //Sum entire array
        List<Integer> ans=Arrays.asList(6,2,3,4,5,66);
        System.out.println("\n");
        Optional<Integer> reuslt=ans.stream().reduce((x, y)->x+y);//Optional class is used ot handle empty value
        System.out.println(ans.stream().reduce((x,y)->x+y));
        ...
        Optional<Integer>sum =ans.stream().reduce(Integer::sum);
        ...
        Boolean greaterThan=ans.stream().anyMatch(x->x>10);
        Boolean allSatisfies=ans.stream().allMatch(x->x>50);

        //AnyMatch , findFirst , FindAny
        System.out.println(ans.stream().findAny().get());
